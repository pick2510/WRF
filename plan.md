...
- `advance_w` has a working OpenACC implementation and uses explicit host/device synchronization at CPU/GPU boundaries.
...
1. Replace `advance_w`'s broad `kernels` region with explicit OpenACC loop regions. Preserve sequential vertical tridiagonal sweeps while parallelizing independent horizontal work.
2. Reduce synchronization around `advance_w`. Port the immediately adjacent acoustic routines first, then keep their shared fields resident across calls.
